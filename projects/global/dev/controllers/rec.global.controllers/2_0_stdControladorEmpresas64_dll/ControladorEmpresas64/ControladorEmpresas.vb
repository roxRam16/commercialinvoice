...
'Imports gsol.krom.MenuDinamico.MenuDinamicoWeb
...
#Region "PROPIEDADES PRIVADAS"
    ...
    Private _secuencia As Rec.Globals.Utils.ISecuencia
...
#Region "PROPIEDADES PUBLICAS"
    Public Property TipoEmpresa As IControladorEmpresas.TiposEmpresas _
    ...
    Public Property Modalidad As IControladorEmpresas.Modalidades _
        Implements IControladorEmpresas.Modalidad

    Public Property PaisEmpresa As String _
        Implements IControladorEmpresas.PaisEmpresa
...
#Region "MÉTODOS PRIVADOS"
    Private Function GenerarSecuencia(Optional ByVal tipoSecuencia_ As Int32 = 1) As Rec.Globals.Utils.Secuencia

        _controladorSecuencias = New ControladorSecuencia

        _secuencia = New Rec.Globals.Utils.Secuencia

        Dim secuenciaEmpresa_ = Syn.Nucleo.Recursos.SecuenciasRecursos.Empresas

        Estado = _controladorSecuencias.Generar(secuenciaEmpresa_, tipoSecuencia_, 1, 1, 1, 1)

        With Estado

            If .Status = TagWatcher.TypeStatus.Ok Then

                _secuencia = DirectCast(.ObjectReturned, Rec.Globals.Utils.Secuencia)
            ...
            If .Status = TagWatcher.TypeStatus.Ok Then
                ...
                For Each item_ In .ObjectReturned
                    ...
                    With _paisInterno
                        ...
                        .nombrepaisesp = item_.cveISO3 & " - " & item_.paisPresentacion
    ...
    Private Function ArchivarRegistro(ByVal tipo_ As IControladorEmpresas.TipoEstructura,
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}

                    Dim operationsDB_ As IMongoCollection(Of EmpresaNacional) = iEnlace_.GetMongoCollection(Of EmpresaNacional)(GetType(EmpresaNacional).Name)

                    Dim setStructureOfSubs_ As UpdateDefinition(Of EmpresaNacional)

                    Dim result_ As UpdateResult = Nothing

                    Dim filter_ As FilterDefinition(Of EmpresaNacional) = Builders(Of EmpresaNacional).Filter.Eq(Function(x) x.razonsocial, filtro_)
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    If objetoRetorno_ Then
                        ...
                        Dim filter_ As FilterDefinition(Of EmpresaNacional) = Builders(Of EmpresaNacional).Filter.Eq(Function(x) x.razonsocial, empresaNacional_.razonsocial)
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim operationsDB_ As IMongoCollection(Of EmpresaInternacional) = iEnlace_.GetMongoCollection(Of EmpresaInternacional)(GetType(EmpresaInternacional).Name)
                    ...
                    If objetoRetorno_ Then
                        ...
                        Dim filter_ As FilterDefinition(Of EmpresaInternacional) = Builders(Of EmpresaInternacional).Filter.Eq(Function(x) x.razonsocial,
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim filter_ As FilterDefinition(Of EmpresaNacional) = Builders(Of EmpresaNacional).Filter.And(
                                Builders(Of EmpresaNacional).Filter.Eq(Of ObjectId)("_id", empresaNacional_._id),
    ...
    Private Function ModificarEmpresaInternacional(ByRef empresaInternacional_ As EmpresaInternacional,
                                                   ...
                                                   Optional ByVal crearEstructuraDomicilio_ As Boolean = False) As TagWatcher
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    'Dim paisExiste = empresaInternacional_.paisesdomicilios.Any(Function(p) p.pais = PaisEmpresa)

                    'If Not paisExiste Then
                    '    crearEstructuraDomicilio_ = True
                    'End If

                    ' >>>>> CONSULTA A MONGO PARA VER SI YA EXISTE EL PAIS EN LA BASE <<<<<
                    Dim filterPaisExiste = Builders(Of EmpresaInternacional).Filter.And(
                            Builders(Of EmpresaInternacional).Filter.Eq(Function(x) x._id, empresaInternacional_._id),
                            Builders(Of EmpresaInternacional).Filter.ElemMatch(Of PaisDomicilio)(
                                Function(x) x.paisesdomicilios,
                                Builders(Of PaisDomicilio).Filter.Eq(Function(pd) pd.pais, PaisEmpresa)
                            )
                        )

                    Dim paisExisteEnMongo = operationsDB_.Find(filterPaisExiste).Any()

                    ' Si no existe en Mongo, entonces debemos agregar el país completo
                    If Not paisExisteEnMongo Then
                        crearEstructuraDomicilio_ = True
                    End If

                    Dim filter_ As FilterDefinition(Of EmpresaInternacional)
                    ...
                    With empresaInternacional_
                        ...
                        filter_ = Builders(Of EmpresaInternacional).Filter.And(
                                Builders(Of EmpresaInternacional).Filter.Eq(Function(x) x._id, ._id),
                                Builders(Of EmpresaInternacional).Filter.Eq(Function(x) x.estado, 1))

                        ' Manejo del país/domicilio
                        If domicilioNuevo_.Count > 0 Then

                            If crearEstructuraDomicilio_ Then

                                Dim estructuraPaisDomicilio_ As PaisDomicilio =
                                .paisesdomicilios.First(Function(x) x.pais = PaisEmpresa)
                        ...
                        ' Contactos
                        If .contactos IsNot Nothing AndAlso .contactos.Count > 0 AndAlso .contactos.Last IsNot Nothing Then
                            setStructureOfSubs_ = setStructureOfSubs_.AddToSet("contactos", DirectCast(.contactos.Last, Contacto))
                        End If

                        ' Bus
                        If .bus IsNot Nothing AndAlso .bus.Count > 0 AndAlso .bus.Last IsNot Nothing Then
                        ...
                        Else
                            Try
                                result_ = operationsDB_.UpdateOneAsync(filter_, setStructureOfSubs_, New UpdateOptions With {.IsUpsert = True}).Result
                            Catch ex As Exception
                                ' Reintenta creando la estructura completa
                                Try
                    ...
                    If result_ IsNot Nothing Then
                        If result_.ModifiedCount > 0 OrElse result_.UpsertedId IsNot Nothing Then
                            .SetOK()
                        ElseIf result_.MatchedCount = 1 Then
                            .SetOK()
                        Else
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim operationsdb_ As IMongoCollection(Of T) = iEnlace_.GetMongoCollection(Of T)(GetType(T).Name)

                    Dim filter_ As FilterDefinition(Of T) = Builders(Of T).Filter.And(
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim filter_ As FilterDefinition(Of T) = Builders(Of T).Filter.Eq(Of ObjectId)("_id", cveEmpresa_) And
                                                            Builders(Of T).Filter.Eq(Of Boolean)("estado", 1)
    ...
    Private Function ConsultarEmpresaInterna(ByVal cveEmpresa_ As ObjectId) As TagWatcher
    ...
    Private Function ConsultarDomiciliosExternos(Of T)(ByVal cveEmpresa_ As ObjectId) As TagWatcher
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim filter_ As FilterDefinition(Of T) = Builders(Of T).Filter.Eq(Of ObjectId)("_id", cveEmpresa_) _
                                  And Builders(Of T).Filter.Eq(Of Boolean)("estado", 1)

                    Dim result_ = operationsDB_.Find(filter_).Limit(1).ToList().AsEnumerable

                    Dim empresa_ As Empresa = DirectCast(result_.FirstOrDefault, IEmpresa)

                    ListaDomicilios = New List(Of Domicilio)

                    For Each domicilio_ As Domicilio In empresa_.paisesdomicilios.
    ...
    Private Function ConsultarDomiciliosInternos(ByVal cveEmpresa_ As ObjectId, ByVal pais_ As String) As TagWatcher
    ...
    Private Function ConsultarDomicilioExterno(Of T)(ByVal idEmpresa_ As ObjectId, ByVal idDomicilio_ As ObjectId) As TagWatcher
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim filter_ As FilterDefinition(Of T) = Builders(Of T).Filter.Eq(Of ObjectId)("_id", idEmpresa_) And
                                  Builders(Of T).Filter.Eq(Of Boolean)("estado", 1)
    ...
    Private Function ArchivarEmpresas(Of T)(ByRef listIdEmpresa_ As List(Of ObjectId)) As TagWatcher
        ...
        With Estado
            ...
            Try
                ...
                Using iEnlace_ As IEnlaceDatos = New EnlaceDatos(26) With {.EspacioTrabajo = _espacioTrabajo}
                    ...
                    Dim filter_ As FilterDefinition(Of T) = Builders(Of T).Filter.In(Of ObjectId)("_id", listIdEmpresa_)
