export class WCCatalog extends HTMLTableElement {
    ...
    connectedCallback() {
        ...
        const printBtn = this.component.querySelector('.__print') || false;
        ...
        if (printBtn) printBtn.addEventListener('click', e => this.printRow(e));
    ...
    setControlsEvents(region_) {
        ...
        const inputs = region_.querySelectorAll(
            'tr.__row input[id], tr.__row textarea[id], tr.__row select[id]' || []
        );

        inputs.forEach(input => {

            input.addEventListener('input', e => {
                this.syncSingleInput(e.target);
            });
            ...
            input.addEventListener('change', e => {
                ...
                if (!e.target.hasAttribute('in-process')) {
                    ...
                    e.target.removeAttribute('in-process');
    ...
    syncSingleInput(input) {

        const td = input.closest('td');
        const tr = input.closest('tr');
        const rowIndex = tr.rowIndex - 2;
        const key = td.id;

        let json;
        try {
            json = JSON.parse(this.jsonString.value);
        } catch {
            return;
        }

        if (!json.Items || !json.Items[rowIndex]) return;


        if (input.type === 'checkbox') {
            json.Items[rowIndex][key] = input.checked;
        }
        else if (input.tagName === 'SELECT') {

            json.Items[rowIndex][key] = {
                Value: input.value,
                Text: input.options[input.selectedIndex]?.text || ''
            };
        }
        else {
            json.Items[rowIndex][key] = input.value;
        }
    ...
    printRow(e) {
        ...
        tableRows_.forEach((e) => {
            ...
            if (Number(e.value) != 0) {
                ...
                json_.Items[e.value - 1].isChecked = true;
    ...
    deleteRow(e) {
        ...
        const tableRows_ = this.querySelectorAll(
            'tr.__row td:first-child input:not(.__checks):checked'
        );

        const json_ = JSON.parse(this.jsonString.value);

        tableRows_.forEach((el) => {

            const rowIndex = el.closest('tr').rowIndex - 2;

            if (json_.Items[rowIndex]) {
                json_.Items[rowIndex].isChecked = true;
            }
