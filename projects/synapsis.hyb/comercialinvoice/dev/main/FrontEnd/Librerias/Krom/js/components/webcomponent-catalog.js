export class WCCatalog extends HTMLTableElement {
    ...
    setControlsEvents(region_) {
        ...
        inputs.addEventListener('change', (e) => {
            ...
            if (e.target.type === 'checkbox' && !e.target.closest('label.wc-switch')) {
                return;
            }
    ...
    prepareJsonData() {
        ...
        try {
            ...
            const rows = this.querySelectorAll('tbody tr.__row');

            rows.forEach((tr, index) => {

                json.Items[index] = json.Items[index] || {};
                const row_ = json.Items[index];
                ...
                tr.querySelectorAll('td').forEach((td) => {
                    ...
                    if (control_.type.toLowerCase() === 'checkbox') {
                        ...
                        if (control_.id === 'id_') {
                            row_[td.id] = Number(control_.value);
                        } else {
                            row_[td.id] = Boolean(control_.checked);
                        }
