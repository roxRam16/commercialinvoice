...
// Impide pegar letras
$(document).on('paste', '.numeric', function (evt) {
...
// Limpieza final post-evento (por si algo escapa)
$(document).on('input', '.numeric', function () {
    var input = $(this);
    var clean = input.val().replace(/[^\d]/g, '');
...
$(document).ready(function () {
    ...
    attachCurrencyFormatter('.currencyDecimal', formatCurrencyDecimal);
    attachCurrencyFormatter('.currencyInteger', formatCurrencyInterger);
    attachCurrencyFormatter('.currency', formatCurrency);

    if (typeof Sys !== 'undefined' && Sys.WebForms) {
        Sys.WebForms.PageRequestManager.getInstance().add_endRequest(function () {
            reformatCurrencyInputs();
        });
    }

});
function reformatCurrencyInputs() {
    $('.currencyDecimal').each(function () {
        formatCurrencyDecimal($(this), 'blur');
    });

    $('.currencyInteger').each(function () {
        formatCurrencyInterger($(this), 'blur');
    });
...
function attachCurrencyFormatter(selector, formatFunction) {
...
function formatCurrency(input, blur) {
    ...
    var decimalsAttr = input.attr("decimals");
    var hasDecimals = decimalsAttr !== undefined;
    var decimals = hasDecimals ? parseInt(decimalsAttr, 10) : null;
...
$('body').on('click', '.lock-input', function (e) {
    ...
    if ($(this).attr('data-no-permission')) {
        e.preventDefault();

        const container = $(this).closest('.wc-input.__component');

        const input = container.find('input, textarea').first();

        const validationContainer = container.children('div').not('.lock-input').first();

        container.addClass('no-permission');

        if (validationContainer.find('.no-permission-msg').length === 0) {
            validationContainer.append('<span class="no-permission-msg">No tienes permisos de edición.</span>');
        }
    ...
    if ($(this).get(0).hasAttribute('pbref')) {
        ...
        DisplayPrompt('Desbloquear', 'Ingrese el código de seguridad', null, null, (value_) => {
