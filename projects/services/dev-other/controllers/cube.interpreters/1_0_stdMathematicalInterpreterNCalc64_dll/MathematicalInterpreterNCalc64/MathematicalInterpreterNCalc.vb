...
#Region "Methods"
    ...
    Public Function RunExpression(Of T)(expression_ As String,
                                        ...
                                        Optional formulaFieldName_ As String = Nothing) As T _
        ...
        For Each constanvalue_ In constantValues_
            ...
            If pointIndex <> -1 Then
                ...
                If Integer.TryParse(fieldName_.Substring(pointIndex + 1), integerParse_) Then
                    ...
                    If Regex.IsMatch(fieldName_, patron_) Then

                        fieldName_ = fieldName_.Substring(0, fieldName_.LastIndexOf("."))
    ...
    Private Function GetExpression(Of T)(expression_ As String,
                                  ...
                                  Optional prederIndex_ As Int32 = 0,
                                  Optional formulaFieldName_ As String = Nothing) _
                                  As String
            ...
            Else
                ...
                Else
                    ...
                    If element_(element_.Length - 1) = "}" Then
                        ...
                        If lastRangePosition_ <> -1 Then

                            Dim finalPosition_ As String = range_.Substring(lastRangePosition_ + 1)

                            Dim tryParseInteger_ As Int16

                            If Integer.TryParse(finalPosition_, tryParseInteger_) Then

                                finalPosition_ = tryParseInteger_.ToString
...
finalExpression_.Length - 1)
                            Else
                                ...
                                finalExpression_ &= SetValuesOperands(curedElement_,
                                                                         ...
                                                                         values_,
                                                                         False,
                                                                         formulaFieldName_:=formulaFieldName_)
    ...
    Function SetValuesOperands(Of T)(originalExpression_ As String,
                                       ...
                                       values_ As Dictionary(Of String, T),
                                       skip_ As Boolean,
                                       Optional brackets_ As Boolean = True,
                                       Optional formulaFieldName_ As String = Nothing) _
                                       As String
                            ...
                            Else
                                ...
                                For maxposicion_ = position_ To 0 Step -1
                                    ...
                                    Else
                                        ...
                                        Dim foundList_ As New List(Of String)

                                        If formulaFieldName_ IsNot Nothing Then

                                            foundList_ = values_.Keys.Where(Function(ch) ch.Contains(operand_ & ObtenerIndice(formulaFieldName_))).ToList

                                        End If



                                        If foundList_.Count > 0 Then

                                            operand_ = foundList_(0)

                                            'operand_ = operand_.Substring(0, operand_.LastIndexOf("."))
                                        ...
                                        Else
                                            ...
                                            foundList_ = values_.Keys.Where(Function(ch) ch.Contains(operand_)).ToList

                                            If foundList_.Count >= maxposicion_ + 1 Then

                                                operand_ = foundList_(maxposicion_)
        ...
        Return VerificaElementos(Of T)(expression_, values_)

    End Function

    Function VerificaElementos(Of T)(expression_ As String, values_ As Dictionary(Of String, T)) As String

        Dim newExpression_ = expression_

        Dim operand_ As String

        Dim bracketPosition_ = 0

        Dim operandDictionary_ As New Dictionary(Of String, String)
        ...
        For Each character_ In expression_
            ...
            Select Case character_
                ...
                Case "["
                    ...
                    operand_ = expression_.Substring(bracketPosition_ + 1)

                    operand_ = operand_.Substring(0, operand_.IndexOf("]"))

                    If Not values_.ContainsKey(operand_) Then

                        operandDictionary_(operand_) = ObtenerIndice(operand_)

                    Else

                        Dim temIndex_ = ObtenerIndice(operand_)

                        If Regex.IsMatch(temIndex_, "\.\d+.+") Then

                            finalIndex_ = temIndex_
        ...
        For Each key_ In operandDictionary_.Keys

            If Regex.IsMatch(key_, "\.\d+.+") Then

                If Not values_.ContainsKey(key_) Then

                    If finalIndex_ <> "" Then

                        Dim newKey_ = key_.Substring(0, key_.IndexOf(operandDictionary_(key_))) & finalIndex_.Substring(0, finalIndex_.LastIndexOf("."))

                        If values_.ContainsKey(newKey_) Then

                            newExpression_ = newExpression_.Replace(key_, newKey_)
                ...
                Case " ", "+", "-", "*", "/", "^", "?", "(", ")", ",", ">", "=", "<"
                    ...
                    If Not isNumber_ Then
                        ...
                        If operand_ <> "" Then
                            ...
                            If pointLastPosition_ > -1 Then
                                ...
                                If Integer.TryParse(operand_.Substring(pointLastPosition_ + 1),
                                    ...
                                    If Regex.IsMatch(operand_, "\.\d+.\d+") Then

                                        operand_ = Regex.Replace(operand_, "\.\d+.\d+", "")

                                    Else

                                        operand_ = Regex.Replace(operand_, "\.\d+", "")
                        ...
                        If operand_ <> "" Then
                            ...
                            For Each operandValues_ In GetParams(operand_)

                                'Dim operandValuesAux_ = If(operandValues_.Count(Function(c) c = ".") = 2

                                Dim matches_ = values_.Keys.Where(Function(e) If(e.Count(Function(c) c = ".") >= 2, e.Contains(operandValues_), False)).Count
        ...
        Dim procedure_ = Sub(functionName_ As String,
                             ...
                             Select Case functionName_.ToUpper
                                 ...
                                 Case "BUSCAR"
                                     ...
                                     Dim position_ As Int32 = secondParameter_.indexOf(firstParameter_)

                                     If position_ >= 0 Then

                                         functionParameters_.Result = thirdParameter_(position_)
                                     ...
                                     If functionParameters_.Parameters.Count = 3 Then
                                         ...
                                         If TypeOf elementSecond_ Is String Then
                                             ...
                                             For Each element_ In elements_
                                                 ...
                                                 If Not dictionary_.ContainsKey(key_) Then
                                 ...
                                 Case "HALLAR"
                                     ...
                                     Dim index_ As Int16 = secondParameter_.IndexOf(firstParameter_)
                                 ...
                                 Case "RANGOS"
                                     ...
                                     Dim firstParameter_ As String = functionParameters_.
                                                    Parameters.
                                                    First.ParsedExpression.ToString

                                     Dim inicial_ As Int16 = functionParameters_.
                                                    Parameters(1).Evaluate
                                     Dim final_ As String = functionParameters_.Parameters.Last.ParsedExpression.ToString
                                 ...
                                 Case "ROOM"
                                     ...
                                     If paramPosition_ > 0 And paramPosition_ < functionParameters_.Parameters.Count - 1 Then
                                         ...
                                         For position_ = 1 To functionParameters_.Parameters.Count - paramPosition_
                                             ...
                                             If foundParameter_ IsNot Nothing Then

                                                 '    If values_.ContainsKey(lastParameter_.ToString & "." & position_) Then

                                                 '        functionParameters_.Parameters.insert(functionParameters_.Parameters(0))
                                 ...
                                 Case "SUMAR.SI"
                                     ...
                                     Dim expressionExecute_ = lastParameter_.
                                                  ParsedExpression.
                                                  ToString

                                     Dim paramsNew_ = GetParams(lastParameter_.
                                                  ParsedExpression.
                                                  ToString.Replace("!=", "<>"))
                                     ...
                                     For Each parameter_ In functionParameters_.
                                         ...
                                         expressionExecute_ = lastParameter_.
                                                  ParsedExpression.
                                                  ToString

                                         For Each paramNew_ In paramsNew_

                                             Dim paramTemp_ = paramNew_

                                             If Regex.IsMatch(paramTemp_, "\.\d+.\d+") Then

                                                 paramTemp_ = Regex.Replace(paramTemp_, "\.\d+.\d+", "")

                                             Else

                                                 paramTemp_ = Regex.Replace(paramTemp_, "\.\d+", "")

                                             End If

                                             paramTemp_ = paramTemp_.Replace("[", "").Replace("]", "")

                                             Dim generalNameNew_ = SetValuesOperands(paramTemp_,
                                                                                    operandCount_,
                                                                                    values_,
                                                                                    False)

                                             expressionExecute_ = expressionExecute_.Replace(paramNew_, generalNameNew_)
                                         ...
                                         If evaluate_ Then
                                             ...
                                             Dim expressionSI_ = New NCalc.Expression(expressionExecute_)
                                             ...
                                             expressionSI_.Parameters = values_

                                             AddHandler expressionSI_.EvaluateFunction, RunFunctionHandler(values_)
