...
Public Class VRUVA006_SYNA22CustomsDeclarationDoc
    Inherits ValidationRUVA

    Sub New()

        _validationtarget = IValidationRoute.ValidationRoutes.RUVA6
    ...
    Protected Overrides Function LoadInitialStockElement() As ValidatorReport
        ...
        '#3
        If [Set](ANS1, CA_MEDIO_TRANSPORTE, breakOnEmpty_:=True, runAt_:=IValidationRoute.RunAt.LOCALCONDITIONS, conditions_:=New List(Of String) From {"=", "98"}) Then Return _report
        ...
        '#7
        If [Set](ANS1, CA_MEDIO_TRANSPORTE_ARRIBO, breakOnEmpty_:=True, runAt_:=IValidationRoute.RunAt.LOCALCONDITIONS, conditions_:=New List(Of String) From {"=", "98"}) Then Return _report

        '#8
        If [Set](ANS1, CA_MEDIO_TRANSPORTE_SALIDA, breakOnEmpty_:=True, runAt_:=IValidationRoute.RunAt.LOCALCONDITIONS, conditions_:=New List(Of String) From {"=", "98"}) Then Return _report

        '#9
        If [Set](ANS1, CA_TIPO_OPERACION, breakOnEmpty_:=True, runAt_:=IValidationRoute.RunAt.LOCALCONDITIONS, conditions_:=New List(Of String) From {"=", "2"}) Then Return _report

        '#10
        If [Set](ANS1, CA_CVE_PEDIMENTO, breakOnEmpty_:=True, runAt_:=IValidationRoute.RunAt.LOCALCONDITIONS, conditions_:=New List(Of String) From {"=", "GC"}) Then Return _report

        '#11
        If [Set](ANS1, CA_REGIMEN, breakOnEmpty_:=True, runAt_:=IValidationRoute.RunAt.LOCALCONDITIONS, conditions_:=New List(Of String) From {"=", "EXD"}) Then Return _report
