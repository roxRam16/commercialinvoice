...
#End Region
    ...
    Private Sub RunRules(Of T)(rules_ As String,
                               ...
                               Optional formulaFieldName_ As String = Nothing)
        ...
        Else
            ...
            _status = New TagWatcher() With {.ObjectReturned = _interpreter.RunExpression(Of T)(rules_, params_, preferIndex_, formulaFieldName_)}
...
updateDefinitionResource_,
    ...
    Public Function CamposExcelMongo(excelFilePath_ As String) As String Implements ICubeController.CamposExcelMongo
        ...
        Using _enlaceDatos As IEnlaceDatos = New EnlaceDatos(ICubeController.EnlaceSax.Synapsis)

            _enlaceDatos.GetMongoCollection(Of BsonDocument)("[SynapsisN].[dbo].[Vt022AduanaSeccionA01]").
                                 Aggregate.
                                 ToList.ForEach(Sub(element_)
        ...
        '    Dim notfound_ As String = ""

        '    For Each elemento_ In result_

        '        Dim sectiosnexcel_ = elemento_("SECCIONES").Split("|")

        '        For Each section_ In sectiosnexcel_

        '            or_ &= "{$and:[{sectionfield:'" & elemento_("CAMPO") & "'},{sectionexcel:'" & section_ & "'}]},"

        '            Using _enlaceDatos As IEnlaceDatos = New EnlaceDatos(ICubeController.EnlaceSax.Cube)

        '                If _enlaceDatos.GetMongoCollectionByRootId(Of validfields)(ICubeController.RootCube.ValidFields).
        '                         Aggregate.
        '                         Match("{$and:[{sectionfield:'" & elemento_("CAMPO") & "'},{sectionexcel:'" & section_ & "'}]}").
        '                         ToList.Count = 0 Then

        '                    notfound_ &= elemento_("CAMPO") & "|" & section_ & vbCrLf
    ...
    Public Function CamposDictionaryMongo(dictionary As Dictionary(Of String, String)) As String Implements ICubeController.CamposDictionaryMongo

        Dim notfound_ As String = ""

        For Each elemento_ In dictionary.Keys

            Dim secciones_ = dictionary(elemento_).Split(",")

            For Each seccion_ In secciones_

                Using _enlaceDatos As IEnlaceDatos = New EnlaceDatos(ICubeController.EnlaceSax.Cube)

                    If _enlaceDatos.GetMongoCollectionByRootId(Of validfields)(ICubeController.RootCube.ValidFields).
                                 Aggregate.
                                 Match("{$and:[{sectionfield:'" & elemento_ & "'},{sectionexcel:'" & seccion_ & "'}]}").
                                 ToList.Count = 0 Then

                        notfound_ &= elemento_ & "|" & seccion_ & vbCrLf
    ...
    Public Function RunRoom(Of T)(roomname_ As String,
                                  ...
                                  Optional preferIndex_ As Int32? = Nothing,
                                  Optional formulaFieldName As String = Nothing) As ValidatorReport Implements ICubeController.RunRoom
        ...
        If roomname_ = "A22.CA_EFECTIVO" Then
                ...
                Else
                    ...
                    For Each param_ In _rooms(0).addresses(1).ref.Skip(1)
                        ...
                        For Each key_ In params_.Keys
                            ...
                            Else
                                ...
                                If Regex.IsMatch(keyReg_, "\.\d+.\d+") Then

                                    keyReg_ = Regex.Replace(keyReg_, "\.\d+.\d+", "")

                                Else

                                    keyReg_ = Regex.Replace(keyReg_, "\.\d+", "")

                                End If

                                If params_.Keys.Where(Function(e) e.Contains(keyReg_)).ToList.Count > 0 Then
                    ...
                    If found_ Then
                        ...
                        Else
                            ...
                            RunRules(Of T)(_rooms(0).rules, params_, preferIndex_, formulaFieldName)
